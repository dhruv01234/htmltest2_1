function showpages(filename, displayName) {
    let curbook = "";
    ...
    xmlht.onreadystatechange = function () {
        if (xmlht.readyState == 4 && xmlht.status == 200) {
            curbook = xmlht.responseText;
            getDocStats(curbook);
            curbook = curbook.replace(/(?:\r\n|\r|\n)/g, '<br>');
            document.getElementById("fileContent").innerHTML = curbook;
...
function getDocStats(fileContent) {
    var totallen = document.getElementById("docLength");
    var totalwords = document.getElementById("wordCount");
    let text = fileContent.toLowerCase();
    let wordstable = text.match(/\b\S+\b/g);
    let wordDictionary = {};
    var uncommonWords = [];
    uncommonWords = filterStopWords(wordstable);
    ...
    totallen.innerText = "Document Length: " + text.length;
    totalwords.innerText = "Word Count: " + wordstable.length;
...
function filterStopWords(wordstable) {
    ...
    for (i = 0; i < wordstable.length; i++) {
        word = wordstable[i].trim().toLowerCase();
        if (!commonObj[word]) {
